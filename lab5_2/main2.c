...
bool log_data_check(Log *log) {
    ...
    // uint8_t temp[64] = {0};
    // memcpy(temp, read_log_buf, end_index);
    // temp[end_index] = read_log_buf[end_index + 1];
    // temp[end_index + 1] = read_log_buf[end_index + 2];
...
int find_log_last_point_num(Log *log) {
    log->read_log_buf[0] = 0x00;
    log->read_log_buf[1] = 0x00;
